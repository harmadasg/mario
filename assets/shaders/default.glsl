...
void main()
{
    // This may look bad, but it is intentional, openGL minimum spec does not require dynamic indexing
    // with variables into texture arrays, so this switch is required on AMD GPUs.
    if (fTexId > 0) {
        int id = int(fTexId);
        switch (id) {
            case 0:
